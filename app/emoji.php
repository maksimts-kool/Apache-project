...
$category = getCategoryBySlug($pdo, $emoji['category']);
$category_label = $category ? buildCategoryLabel($category) : $emoji['category'];
    ...
    <main class="emoji-detail">
        <div class="emoji-detail-card">
            ...
            <div class="detail-meta">
                ...
                <span>•</span>
                <span><?= htmlspecialchars($category_label) ?></span>
                <span>•</span>
                <span>📋 <span class="copy-count"
                        data-id="<?= $emoji['id'] ?>"><?= number_format($emoji['downloads']) ?></span></span>
                <span>•</span>
            ...
            <div class="detail-actions">
                <button class="btn btn-primary btn-copy" style="padding: 12px 32px; font-size: 16px;"
                    data-symbol="<?= htmlspecialchars($emoji['symbol'], ENT_QUOTES) ?>" data-id="<?= $emoji['id'] ?>">
                    📋 Copy Emoji
                </button>
                <button class="btn btn-secondary btn-like<?= $user_liked ? ' liked' : '' ?>"
                    style="padding: 12px 24px; font-size: 16px;" data-id="<?= $emoji['id'] ?>">
                    <span class="like-icon"><?= $user_liked ? '♥' : '♡' ?></span> <span
                        class="like-count"><?= number_format($emoji['likes']) ?></span>
                </button>
                <?php if ($is_owner): ?>
                <a href="/upload.php?id=<?= $emoji['id'] ?>" class="btn btn-secondary"
                    style="padding: 12px 24px; font-size: 16px;">✏️ Edit</a>
                <?php endif; ?>
            </div>
        </div>
    </main>
